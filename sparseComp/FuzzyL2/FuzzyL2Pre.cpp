...
Proto sparse_comp::fuzzy_l2_pre::Sender<tr, t, d, delta, ssp>::offline(
    Socket &sock, array<point, t> &points) {

  MC_BEGIN(Proto, this, &sock, &points, prt = Proto());
  // Create the SpL2 instance whose preprocessing state
  // will be reused in the online phase.
  this->spL2Sender = new SpL2SenderPre<rcvr_cell_count, t, delta, ssp>(
      *(this->prng), *(this->aes));
  ...
  sparse_comp::spatial_hash<t>(*(this->aes), points, this->point_hashs, d,
                               delta);
  // Convert the input points to the representation required by SpL2.
  this->in_values = new array<array<uint32_t, d>, t>();

  sndr_points_to_in_values<t, d>(points, *(this->in_values));

  // Perform the input-independent cryptographic preprocessing of SpL2.
  prt = this->spL2Sender->offline(sock);
  MC_AWAIT(prt);

  MC_END();
}
template <size_t ts, size_t t, size_t d, uint8_t delta, uint8_t ssp>
Proto sparse_comp::fuzzy_l2_pre::Receiver<ts, t, d, delta, ssp>::offline(
    Socket &sock, array<point, t> &points) {
...
template <size_t tr, size_t t, size_t d, uint8_t delta, uint8_t ssp>
Proto sparse_comp::fuzzy_l2_pre::Sender<tr, t, d, delta, ssp>::online(
    Socket &sock, array<point, t> &points) {
  ...
  // Execute the online SpL2 evaluation using the preprocessing
  // generated in the offline phase.
  prt = this->spL2Sender->online(sock, this->point_hashs, *(this->in_values),
                                *out_vec_shares);
  ...
  delete this->spL2Sender;
  this->spL2Sender = nullptr;
...
template <size_t ts, size_t t, size_t d, uint8_t delta, uint8_t ssp>
Proto sparse_comp::fuzzy_l2_pre::Receiver<ts, t, d, delta, ssp>::online(
  ...
  // Execute the online SpL2 evaluation using the preprocessing
  // generated in the offline phase.
  prt = this->spL2Receiver->online(sock, this->cells, *(this->in_values),
                                  *out_vec_shares);
  ...
  delete this->spL2Receiver;
  this->spL2Receiver = nullptr;
