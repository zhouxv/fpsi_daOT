...
#include "../sparseComp/FuzzyLInf/FuzzyLInfPre.h"
...
// Sample a point inside the L-infinity ball used by FuzzyLInf.bench.cpp.
// 按 FuzzyLInf.bench.cpp 的方式，在每个维度独立进行 [-delta, delta] 扰动。
...
void samp_rand_linf_matching_ball(PRNG &prng, point &pt) {
  for (size_t i = 0; i < d; i++) {
    const uint32_t lb = static_cast<uint32_t>(
        std::max<int64_t>(0, static_cast<int64_t>(pt[i]) - delta));
    const uint32_t ub = static_cast<uint32_t>(std::min<int64_t>(
        MAX_U32_BIT_VAL, static_cast<int64_t>(pt[i]) + delta));
...
int run_fuzzylinf_pre(size_t target_matching_points, size_t trait,
  ...
  for (size_t trial = 0; trial < trait; trial++) {
    ...
    sparse_comp::fuzzy_linf_pre::Sender<TR, TS, D, DELTA, ssp> fuzzyLinfSender(
        senderPRNG, aes);
    sparse_comp::fuzzy_linf_pre::Receiver<TS, TR, D, DELTA, ssp> fuzzyLinfRecvr(
        receiverPRNG, aes);

    // Offline phase / 离线阶段
    const auto offline_start = std::chrono::steady_clock::now();
    eval(fuzzyLinfSender.offline(sender_socks[0], *senderPoints),
         fuzzyLinfRecvr.offline(receiver_socks[0], *receiverPoints));
    ...
    eval(fuzzyLinfSender.online(sender_socks[0], *senderPoints),
         fuzzyLinfRecvr.online(receiver_socks[0], *receiverPoints, intersec));
  ...
  const std::string mertric_str = "inf";
...
int dispatch_fuzzylinf_pre(size_t d, uint64_t delta,
  ...
  spdlog::error("Unsupported LInfPre parameters: d={}, delta={}", d, delta);
  return 1;
}

} // namespace

int run_linf_pre(osuCrypto::CLP &cmd) {
