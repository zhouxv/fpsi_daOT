...
#include "../sparseComp/FuzzyL1/FuzzyL1Pre.h"
...
void init_socks(const std::string &ip, uint64_t port,
                std::vector<coproto::Socket> &receiver_socks,
                std::vector<coproto::Socket> &sender_socks) {
...
void shuffle_array(PRNG &prng, std::array<T, n> &arr) {
...
template <typename T> void shuffle_vector(PRNG &prng, std::vector<T> &vec) {
...
void samp_rcvr_pts(AES &, PRNG &prng, std::array<point, tr> &rcvr_points,
                   uint8_t delta) {
  static_assert(d <= point::MAX_DIM);
...
void pick_rand_rcvr_pts(AES &, PRNG &prng, std::array<point, tr> &rcvr_points,
                        std::array<point, ts> &sndr_points, size_t n) {
  std::vector<size_t> rand_idxs(n);
...
// Keep the function name and sampling rule used in FuzzyL1.bench.cpp.
// 保留 FuzzyL1.bench.cpp 中的函数名：总 L1 偏移不超过 delta，再随机分配到各维。
template <size_t d, uint8_t delta>
void samp_rand_linf_matching_ball(PRNG &prng, point &pt) {
  std::array<int64_t, d> offsets{};
  const size_t total_offset =
      prng.get<size_t>() % (static_cast<size_t>(delta) + 1);

  for (size_t i = 0; i < total_offset; i++) {
    const size_t j = prng.get<size_t>() % d;
    offsets[j]++;
  }

  for (size_t i = 0; i < d; i++) {
    const bool orient = prng.get<bool>();
    pt[i] = orient ? static_cast<uint32_t>(pt[i] + offsets[i])
                   : static_cast<uint32_t>(pt[i] - offsets[i]);
  }
}

template <size_t tr, size_t ts, size_t d, uint8_t delta>
void smpl_sndr_rand_pts(AES &aes, PRNG &prng, size_t target_matching_points,
                        std::array<point, tr> &rcvr_points,
  ...
  for (size_t i = target_matching_points; i < ts; i++) {
    sndr_points[i] = gen_rand_sndr_point<d>(prng);
  }
...
void gen_constrained_rand_inputs(block seed, size_t target_matching_points,
  ...
  smpl_sndr_rand_pts<tr, ts, d, delta>(aes, prng, target_matching_points,
                                       rcvr_points, sndr_points);
}

template <size_t TS, size_t TR, size_t D, uint8_t DELTA>
int run_fuzzyl1_pre(size_t target_matching_points, size_t trait,
  ...
  for (size_t trial = 0; trial < trait; trial++) {
    ...
    sparse_comp::fuzzy_l1_pre::Sender<TR, TS, D, DELTA, ssp> fuzzyL1Sender(
        senderPRNG, aes);
    sparse_comp::fuzzy_l1_pre::Receiver<TS, TR, D, DELTA, ssp> fuzzyL1Recvr(
        receiverPRNG, aes);

    // Offline phase / 离线阶段
    const auto offline_start = std::chrono::steady_clock::now();
    eval(fuzzyL1Sender.offline(sender_socks[0], *senderPoints),
         fuzzyL1Recvr.offline(receiver_socks[0], *receiverPoints));
    ...
    eval(fuzzyL1Sender.online(sender_socks[0], *senderPoints),
         fuzzyL1Recvr.online(receiver_socks[0], *receiverPoints, intersec));
  ...
  const std::string mertric_str = "1";
...
template <size_t N>
int dispatch_fuzzyl1_pre(size_t d, uint64_t delta,
                         size_t target_matching_points, size_t trait,
                         const std::string &ip, uint64_t port, bool detail) {
  if (d == 2) {
    switch (delta) {
    case 10:
      return run_fuzzyl1_pre<N, N, 2, 10>(target_matching_points, trait, ip,
                                          port, detail);
    case 60:
      return run_fuzzyl1_pre<N, N, 2, 60>(target_matching_points, trait, ip,
                                          port, detail);
    case 250:
      return run_fuzzyl1_pre<N, N, 2, 250>(target_matching_points, trait, ip,
                                           port, detail);
    }
  }
  ...
  if (d == 6) {
    switch (delta) {
    case 10:
      return run_fuzzyl1_pre<N, N, 6, 10>(target_matching_points, trait, ip,
                                          port, detail);
    case 60:
      return run_fuzzyl1_pre<N, N, 6, 60>(target_matching_points, trait, ip,
                                          port, detail);
    case 250:
      return run_fuzzyl1_pre<N, N, 6, 250>(target_matching_points, trait, ip,
                                           port, detail);
    }
  }
  ...
  if (d == 10) {
    switch (delta) {
    case 10:
      return run_fuzzyl1_pre<N, N, 10, 10>(target_matching_points, trait, ip,
                                           port, detail);
    case 60:
      return run_fuzzyl1_pre<N, N, 10, 60>(target_matching_points, trait, ip,
                                           port, detail);
    case 250:
      return run_fuzzyl1_pre<N, N, 10, 250>(target_matching_points, trait, ip,
                                            port, detail);
    }
  }

  spdlog::error("Unsupported L1Pre parameters: d={}, delta={}", d, delta);
  return 1;
}

} // namespace

int run_l1_pre(osuCrypto::CLP &cmd) {
  ...
  spdlog::info("metric            : l_1");
  spdlog::info("delta             : {}", delta);
  spdlog::info("intersection_size : {}", target_matching_points);
  spdlog::info("trait             : {}", trait);
  spdlog::info("protocol          : L1Pre");
  spdlog::info("***********************************************************");

  switch (n) {
  case 8:
    return dispatch_fuzzyl1_pre<256>(d, delta, target_matching_points, trait,
                                     ip, port, detail);
  case 12:
    return dispatch_fuzzyl1_pre<4096>(d, delta, target_matching_points, trait,
                                      ip, port, detail);
  case 16:
    return dispatch_fuzzyl1_pre<65536>(d, delta, target_matching_points, trait,
                                       ip, port, detail);
  default:
