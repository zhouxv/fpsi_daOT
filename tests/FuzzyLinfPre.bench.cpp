...
#define GENERATE_FUZZYLINF_TEST(n, m, d, delta)                                \
  TEST_CASE("fuzzylinf(n=" #n " m=" #m " d=" #d " delta=" #delta ")",          \
    ...
    (Catch::Benchmark::Chronometer meter) {                                    \
      ...
      sparse_comp::fuzzy_linf_pre::Sender<TR, TS, D, DELTA, ssp>               \
          fuzzyLinfSender(senderPRNG, aes);                                    \
      sparse_comp::fuzzy_linf_pre::Receiver<TS, TR, D, DELTA, ssp>             \
          fuzzyLinfRecvr(receiverPRNG, aes);                                   \
      auto sender_offline_proto =                                              \
          fuzzyLinfSender.offline(socks[0], *senderPoints);                    \
      auto receiver_offline_proto =                                            \
          fuzzyLinfRecvr.offline(socks[1], *receiverPoints);                   \
      sync_wait(when_all_ready(std::move(sender_offline_proto),                \
                               std::move(receiver_offline_proto)));            \
      auto sender_online_proto =                                               \
          fuzzyLinfSender.online(socks[0], *senderPoints);                     \
      auto receiver_online_proto =                                             \
          fuzzyLinfRecvr.online(socks[1], *receiverPoints, intersec);          \
      meter.measure([&sender_online_proto, &receiver_online_proto]() {         \
        sync_wait(when_all_ready(std::move(sender_online_proto),               \
                                 std::move(receiver_online_proto)));           \
      });                                                                      \
      std::vector<point> expected_intersec;                                    \
      expected_linf_intersect<TR, TS, D, DELTA>(                               \
          aes, *receiverPoints, *senderPoints, expected_intersec);             \
      if (is_intersec_correct(aes, intersec, expected_intersec)) {             \
        SUCCEED("Intersection is correct");                                    \
      }                                                                        \
      delete senderPoints;                                                     \
